...
CHARGING_INFO_DIR = OUTPUT_DIR / "charging_info"
DEFAULT_SESSIONS_FILE = INPUT_DIR / "sample_charging_sessions.csv"
DEFAULT_MODELS_FILE = INPUT_DIR / "ev_models_for_simulation.json"


def resolve_path(path_value):
    """Return an absolute path. Relative paths are resolved from this script's folder."""
...
def display_path(path_value):
    """Show paths relative to the script folder when possible."""
...
def load_ev_data(models_file_path=DEFAULT_MODELS_FILE, sessions_file_path=DEFAULT_SESSIONS_FILE):
    # Load EV models data from JSON file
    models_file_path = resolve_path(models_file_path)
    sessions_file_path = resolve_path(sessions_file_path)
...
def process_SOC_pair_with_soc_range(SOC_arrive, SOC_leaving, df_clear, SOC, GENERAL, df_old_total_percentages):
    ...
    for idx, row in df_clear2.iterrows():
        ev_charging_time = 0.0
        ev_power_delivered = 0.0
        ...
        df_charging['charging_power'] = 0.0
        ...
        folder_name = OUTPUT_DIR / "Mode1" / f"SOC{SOC_arrive}to{SOC_leaving}"
        os.makedirs(folder_name, exist_ok=True)
        df_charging.to_csv(folder_name / f"instantaneous_power_sessionID_{idx}_SOC{SOC_arrive}to{SOC_leaving}.csv")
    ...
    os.makedirs(CHARGING_INFO_DIR/ "Mode1", exist_ok=True)
    df_clear2.to_csv(folder_name / f"charging_info_SOC{SOC_arrive}to{SOC_leaving}.csv", index=False)
    df_clear2.to_csv(CHARGING_INFO_DIR / "Mode1" / f"charging_info_SOC{SOC_arrive}to{SOC_leaving}.csv", index=False)
...
def process_SOC_pair_with_variable_soc(df_clear_with_soc, SOC_leaving, SOC, GENERAL, df_old_total_percentages):
    ...
    for idx, row in df_clear2.iterrows():
        ...
        folder_name = OUTPUT_DIR / f"Mode2_SOC_distribution_to{SOC_leaving}"
        os.makedirs(folder_name, exist_ok=True)
        df_charging.to_csv(folder_name / f"instantaneous_power_sessionID_{idx}_SOC{SOC_arrive}to{SOC_leaving}.csv")
    ...
    os.makedirs(CHARGING_INFO_DIR, exist_ok=True)
    df_clear2.to_csv(folder_name / f"charging_info_VariableSOC_to{SOC_leaving}.csv", index=False)
    df_clear2.to_csv(CHARGING_INFO_DIR / f"charging_info_VariableSOC_to{SOC_leaving}.csv", index=False)
...
def process_SOC_pair_with_variable_soc_start_end(df_clear_with_soc, SOC, GENERAL, df_old_total_percentages):
    ...
    for idx, row in df_clear2.iterrows():
        ...
        folder_name = OUTPUT_DIR / "Mode3_SOC_distribution_start_end"
        os.makedirs(folder_name, exist_ok=True)
        df_charging.to_csv(folder_name / f"instantaneous_power_sessionID_{idx}_SOC{SOC_arrive}to{SOC_leaving}.csv")
    ...
    os.makedirs(CHARGING_INFO_DIR, exist_ok=True)
    df_clear2.to_csv(folder_name / "charging_info_VariableSOC_start_end.csv", index=False)
    df_clear2.to_csv(CHARGING_INFO_DIR / "charging_info_VariableSOC_start_end.csv", index=False)
...
if __name__ == "__main__":
    ...
    print("-" * 40)
    print("Default: " +DEFAULT_SESSIONS_FILE.name)
    
    sessions_file_choice = input("Use default file? (y/n): ").lower().strip()
    
    if sessions_file_choice == 'y' or sessions_file_choice == '':
        sessions_file_path = DEFAULT_SESSIONS_FILE
        print(f"Using default file: {display_path(sessions_file_path)}")
    else:
        sessions_file_path = resolve_path(input("Enter path to charging sessions CSV file: ").strip())
        print(f"Using custom file: {display_path(sessions_file_path)}")
    
    # Ask for EV models file path
    print("\n" + "-" * 40)
    print("EV Models File")
    print("-" * 40)
    print("Default: " +DEFAULT_MODELS_FILE.name)
    
    models_file_choice = input("Use default file? (y/n): ").lower().strip()
    
    if models_file_choice == 'y' or models_file_choice == '':
        models_file_path = DEFAULT_MODELS_FILE
        print(f"Using default file: {display_path(models_file_path)}")
    else:
        models_file_path = resolve_path(input("Enter path to EV models JSON file: ").strip())
        print(f"Using custom file: {display_path(models_file_path)}")
    ...
    if cpu_choice == 'y' or cpu_choice == '':
        ...
        try:
            ...
            if cpu_count <= 0 or cpu_count > mp.cpu_count():
                print(f"Invalid CPU count. Using {mp.cpu_count()} cores instead.")
                cpu_count = mp.cpu_count()
            else:
                print(f"Using {cpu_count} CPU cores")
        except ValueError:
            print(f"Invalid input. Using {mp.cpu_count()} cores instead.")
            cpu_count = mp.cpu_count()
    ...
    if SIMULATION_MODE == 1:
        ...
        if dist_choice == 'y' or dist_choice == '':
            ...
            while True:
                ...
                try:
                    ...
                    if soc in SOC_start_probability_distribution:
                        print(f"SOC {soc}% already exists. Replacing with new value.")
            ...
            if not SOC_start_probability_distribution:
                print("No valid distribution entered. Using default.")
        ...
        if abs(total_probability - 1.0) < 0.001:  # Allow small floating point errors
            ...
            print(f"Warning: Probability sum is {total_probability:.3f}, which exceeds 1.0")
                ...
                if normalize_choice in ['y', 'yes']:
                    ...
                    if edit_choice == 'edit':
                        ...
                        if abs(total_probability - 1.0) < 0.001:
                            ...
                            print(f"Sum still exceeds 1.0 ({total_probability:.3f}). Normalizing automatically.")
                            ...
                            print(f"Sum is {total_probability:.3f}, not 1.0. Normalizing automatically.")
                            # Auto-normalize after editing
                            normalized_distribution = {}
                            for soc, prob in SOC_start_probability_distribution.items():
                                normalized_distribution[soc] = prob / total_probability
                        ...
                        print("Proceeding with invalid distribution (sum != 1.0). Normalizing automatically.")
                        # Auto-normalize
                        normalized_distribution = {}
                        for soc, prob in SOC_start_probability_distribution.items():
                            normalized_distribution[soc] = prob / total_probability
            ...
            print(f"Warning: Probability sum is {total_probability:.3f}, not 1.0")
        ...
        if dist_choice_start == 'y' or dist_choice_start == '':
            ...
            print("\nEnter custom SOC_start probability distribution enter 1 value for each SOC percentage press enter to continue:")
            print("Format: SOC_percentage probability (e.g., '20 0.2')")
            print("Enter 'done' when finished")
            print("Note: Probabilities should sum to 1.0")
            ...
            while True:
                ...
                try:
                    ...
                    if charging_amount in SOC_charging_probability_distribution:
                        print(f"Charging amount {charging_amount}% already exists. Replacing with new value.")
        ...
        if abs(total_probability_charging - 1.0) < 0.001:
            ...
            print(f"Warning: Probability sum is {total_probability_charging:.3f}, not 1.0. Normalizing automatically.")
            # Normalize the distribution
            normalized_distribution = {}
